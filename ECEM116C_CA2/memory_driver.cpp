...
struct trace
{
	bool MemR;  //if 1, LOAD instruction
	bool MemW; 	//if 1, STORE instruction
	int adr;  	// address, number between 0 and 4095
	int data; 	// a signed integer value
};

/*
Either implement your memory_controller here or use a separate .cpp/.c file for 
memory_controller and all the other functions inside it (e.g., LW, SW, Search, Evict, etc.)
*/
...
int main (int argc, char* argv[]) // the program runs like this: ./program <filename> <mode>
{
	...
	// AAT = HitTimeL1 + MissRateL1 * (HitTImeVictim + MissRateVictim * (HitTimeL2 + MissRateL2 * HitTimeMemory))
	AAT = myCache.getAAT();

	clog <<"miss rate for victim: " << myCache.getVictimMissRate() << endl;
